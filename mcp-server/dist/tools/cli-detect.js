...
function isVersionValid(cli, version) {
    if (cli === "movement")
        return true;
    return version === REQUIRED_APTOS_VERSION;
}
async function tryCliVersion(binary) {
    try {
        const { stdout } = await execAsync(`${binary} --version`, { timeout: 10000 });
        const version = parseVersion(stdout);
        if (version) {
            const { stdout: whichOut } = await execAsync(process.platform === "win32" ? `where ${binary}` : `which ${binary}`, { timeout: 5000 }).catch(() => ({ stdout: binary }));
...
export async function resolveCli(forceRefresh = false) {
    if (cachedCli && !forceRefresh)
        return cachedCli;
...
export function getCliBinary(info) {
    if (!info.cli)
        return null;
